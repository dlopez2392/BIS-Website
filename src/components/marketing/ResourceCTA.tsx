...
export function ResourceCTA({ kicker, title, body, button, href }: { kicker: string; title: string; body: string; button: string; href: string }) {
  return (
    <section className="mx-auto max-w-6xl px-6 py-16">
      <div className="rounded-card border border-hairline bg-surface-alt p-10 text-center">
        ...
        <Link href={href} className="mt-6 inline-block rounded-ctl bg-primary px-6 py-3 font-bold text-on-primary">{button}</Link>
      </div>
    </section>
  );
}
