...
describe('CallLink', () => {
  it('links to the dialable form of the configured business number', () => {
    ...
    expect(anchor?.getAttribute('href')).toBe('tel:+19567055146');
  });

  it('prints the number in human form', () => {
    const { container } = renderLink();
    expect(container.textContent).toContain('(956) 705-5146');
  });

  it('labels itself for screen readers with the number interpolated', () => {
    const { container } = renderLink();
    expect(container.querySelector('a')?.getAttribute('aria-label')).toBe('Call BIS at (956) 705-5146');
  });

  it('can hide the digits for icon-only placements but stays labelled', () => {
    const { container } = renderLink({ withNumber: false });
    expect(container.textContent).not.toContain('(956) 705-5146');
    expect(container.querySelector('a')?.getAttribute('aria-label')).toBe('Call BIS at (956) 705-5146');
    expect(container.querySelector('a')?.getAttribute('href')).toBe('tel:+19567055146');
  });
