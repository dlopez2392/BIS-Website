...
describe('resolveVisitorContext', () => {
  ...
  it('carries nothing else the client sends: a stray field never reaches the prompt', () => {
    expect(resolveVisitorContext({ locale: 'en', timeZone: 'America/New_York', extra: 1 })).toEqual({
      locale: 'en',
      path: undefined,
    });
  });
