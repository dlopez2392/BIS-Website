...
test('home hero renders headline + both CTAs (EN)', async ({ page }) => {
  await page.goto('/en');
  await expect(page.getByRole('heading', { level: 1, name: /Let us be your Bespoke Intelligent Solution\./i })).toBeVisible();
...
test('home hero renders localized headline (ES)', async ({ page }) => {
  await page.goto('/es');
  await expect(page.getByRole('heading', { level: 1, name: /Deja que seamos tu Solución Inteligente a la Medida\./i })).toBeVisible();
});
